...
public class Game {
    ...
    /**
     * Create the game, initialise its internal map and creates the
     * game_log_file.
     */
    public Game() {
        h = new Hero(5, 5, 30, 15);//O hero tem que ser iniciado primeiro porque preciso dele pra instanciar o trader
        createRooms();
        parser = new Parser();
        finished = false;
        try {//try to create a file named Game_log
    ...
    private void createRooms() {
        ...
        //CREATE WEAPONS
        stick = new AttackWeapon("Stick", 1, "a fragile wooden stick that could be used as a weapon.", 2, 2, 10);
        barrel_top = new DefenseWeapon("Barrel_top", 1, "a barrel top. Could be used for self-defence, if you are completely out of options.", 2, 2, 20);
        club = new AttackWeapon("Club", 2, "a club. It seems to make lots of damage.", 4, 3, 15);
        hammer = new AttackWeapon("Hammer", 2, "a normal hammer. But it could be used for more then just driving nails.", 5, 2, 17);
        wooden_shield = new DefenseWeapon("Wooden_Shield", 2, "a shield made of wood.", 3, 4, 30);
        light_lance = new AttackWeapon("Light_Lance", 3, "a long lance.", 6, 4, 20);
        heavy_wooden_shield = new DefenseWeapon("Heavy_Wooden_Shield", 5, "a heavy shield make of wood. Seems resistent.", 3, 4, 30);
        battle_axe = new AttackWeapon("Battle_Axe", 7, "a heavy axe. Looks deadly.", 10, 5, 20);
        spiked_club = new AttackWeapon("Spiked_Club", 5, "a club with spikes. It looks scary.", 12, 3, 25);
...
//CREATE ITEMS
        ...
        h1 = new HealPotion("Healing_Juice", 2, "a bottle with 'healing juice' written on it", 3);
        h2 = new HealPotion("Healing_Soda", 2, "a bottle with 'healing soda' written on it", 4);
        h3 = new HealPotion("Healing_Tea", 2, "a bottle with 'healing tea' written on it", 5);
        h4 = new HealPotion("Healing_Bottle", 2, "a bottle with 'healing bottle' written on it", 3);
        h5 = new HealPotion("Healing_Liquid", 2, "a bottle with 'healing bottle' written on it", 5);

        s1 = new StrengthPotion("Rat's_Juice", 2, "a bottle with a dark liquid. It's impossible to know what will happen if you drink it", 2);
        s2 = new StrengthPotion("Bat's_Juice", 2, "a bottle with a dark liquid. It's impossible to know what will happen if you drink it", 4);

        p1 = new PoisonPotion("Antidote1", 1, "a bottle with 'drink if you are poisoned, idiot' witten with big letters", 5);
        p2 = new PoisonPotion("Antidote2", 1, "a bottle with 'drink if you are poisoned, idiot' witten with big letters", 5);
        p3 = new PoisonPotion("Antidote3", 1, "a bottle with 'drink if you are poisoned, idiot' witten with big letters", 4);
        p4 = new PoisonPotion("Antidote4", 1, "a bottle with 'drink if you are poisoned, idiot' witten with big letters", 3);
        p5 = new PoisonPotion("Antidote5", 1, "a bottle with 'drink if you are poisoned, idiot' witten with big letters", 2);
    ...
    private boolean processCommand(Command command) {
        ...
        if (commandWord == CommandWord.UNKNOWN) {
            ...
            goRoom(command);
            if (this.currentRoom.getName().equals("THE END")) {//If I am at game's end
                return true;
            }
            ...
            if (!h.isAlive()) {//is hero died, display game over
                System.out.println("\n######################");
    ...
    /**
     * Try to attack a monster. If the monster is not found return an error
     * message
     *
     * @param command
     */
    private void attackMonster(Command command) {
        ...
        if (m.isAlive() && h.isAlive()) {//caso os dois estejam lutando
            ...
            h.lutar(m);//A mecânica de ataque está definda no monstro
    ...
    private boolean talkTo(Command command) {
        ...
        if (Pessoa instanceof KingOdrian) {//Como quando falo com o rei, termina o jogo, preciso desenvolver uma coisa diferente pra ele. A melhor conclusão a que eu cheguei foi o instanceof
            Pessoa.talk();//Exibe todas as falas dele
            finalScene((KingOdrian) Pessoa, h, (Ilea) currentRoom.getPessoa("Ilea"));//vai pra cena final
            currentRoom.emptyMonsters(currentRoom);//mata todos os monstros
            return true;//esse true tá aqui por cause do want to quit lá emcima.
        }

        //Se for qualquer outra pessoa, exibe só as falas dela como de costume
        if (Pessoa == null) {
    ...
    private void equip(Command command) {
        ...
        delay();
        String name = command.getSecondWord();//Aqui é o nome do item que eu quero equipar

        Item i = h.removerItem(name);//tira o item do inventário do herói
        String print = "You equipped ";
        String print_file = h.getNome() + " equipped ";
        if (i == null) {
            System.out.println("You do not have the item you want to equip.");//A mensagem de erro está na função getAttackWeapon
            System.out.println(h.getInventarioString());
            return;
        } //Como o parser não funciona com três palavras pra botar por exemplo equip shield nome-do-shield, usei instanceof aqui também. 
        else if (i instanceof AttackWeapon) {
            if (h.getAttackWeapon() != null) {//se eu já estou equipada com alguma coisa
            ...
            h.equipAttackWeapon((AttackWeapon) i);
            print += i.getNome() + ", a Attack Weapon. ";
            print_file += i.getNome() + ", a Attack Weapon. ";

        } else if (i instanceof DefenseWeapon) {
            if (h.getDefenseWeapon() != null) {//se já tiver equipado alguma coisa
                System.out.println("You are already equipped with " + h.getDefenseWeapon().getNome() + " you have to unequip it first");
                h.pickItem(i);//coloca de volta no inventario do herói
                return;
            }
            h.equipShield((DefenseWeapon) i);
    ...
    private void unequip(Command command) {
        ...
        Item i;
        String print = "You unequipped ";
        String print_file = h.getNome() + " unequipped ";
        //Pra desequipar, verifica se o objeto em questão está em Attackweapon ou em DefenseWeapon
        //if pra attack weapon
        if (h.getAttackWeapon() != null && h.getAttackWeapon().getNome().equals(name)) {//Preciso ter alguma coisa equipada e essa coisa tem que ter o nome que eu quero desequipar
    ...
    public void finalScene(KingOdrian O, Hero h, Ilea i) {
        ...
        if (str.equals("y") || str.equals("Y")) {
            if (O.diceGame(h, i)) {//true eles ganham o jogo, false eles perdem
                ...
                try {
                    TimeUnit.SECONDS.sleep((long) 10);//só pra garantir que vai printar o comando primeiro e depois a ação correspondente.
                } catch (InterruptedException ex) {
                    //não altera nada se não conseguir
                }
    ...
    private void cheat(Command command) //cheats de desenvolvimento
    {
        if (!command.hasSecondWord()) {
